...
- `release-charts.yml` publishes when `chart-ci` passes on a push to `main` that carries a new `Chart.yaml` version: a GitHub Release, the `gh-pages` index, and an OCI chart in GHCR signed with keyless cosign. A rerun, or a merge without a version bump, publishes nothing.
